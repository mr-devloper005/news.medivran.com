...
function EditorialArchive({
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-white text-[#101827]">
        <section className="bg-[#f7f9fc]">
          <div className="pr-container flex flex-col gap-6 py-12 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-sm font-bold uppercase text-[#12366f]">Posts</p>
              <h1 className="mt-3 text-5xl font-extrabold leading-none tracking-[-0.04em] text-[#26313f]">
                {category === 'all' ? label : categoryLabel}
              </h1>
            </div>
            <form action={basePath} className="flex min-w-0 border border-[#9aa0a6] bg-white">
              <select name="category" defaultValue={category} className="h-12 min-w-0 flex-1 bg-transparent px-3 text-sm font-semibold outline-none sm:min-w-64">
                <option value="all">All categories</option>
                {categories.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
              </select>
              <button className="bg-[#12366f] px-5 text-sm font-extrabold text-white">Filter</button>
            </form>
          </div>
        </section>

        <section className="border-y border-[#d2d6dc] bg-white">
          <div className="pr-container flex gap-7 overflow-x-auto py-4 text-sm font-semibold">
            <Link href={basePath} className={category === 'all' ? 'text-[#12366f]' : 'hover:text-[#12366f]'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'text-[#12366f]' : 'whitespace-nowrap hover:text-[#12366f]'}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="pr-container py-14">
            ...
            <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_1fr]">
              <Link href={`${basePath}/${lead.slug}`} className="group block">
                <div className="border border-[#d7d7d7] p-7">
                  <img src={getImage(lead)} alt="" className="aspect-[16/9] w-full object-cover transition duration-500 group-hover:scale-[1.02]" />
                </div>
                <p className="mt-7 text-sm uppercase text-[#26313f]">{lead.publishedAt ? new Date(lead.publishedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }).toUpperCase() : 'JUN 19, 2026'}, 07:04 ET</p>
                <h3 className="mt-2 text-xl font-extrabold uppercase text-[#26313f] group-hover:text-[#12366f]">{lead.title}</h3>
                <p className="mt-4 line-clamp-4 text-base leading-6 text-[#101827]">{getSummary(lead)}</p>
              </Link>
              <div className="grid gap-6">
                {secondary.map((post) => (
                  <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid gap-6 sm:grid-cols-[165px_1fr]">
                    <img src={getImage(post)} alt="" className="aspect-[16/9] w-full border border-[#d7d7d7] object-cover p-1" />
                    <div>
                      <p className="text-sm uppercase text-[#26313f]">{post.publishedAt ? new Date(post.publishedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }).toUpperCase() : 'JUN 19, 2026'}, 09:00 ET</p>
                      <h3 className="mt-2 text-lg font-extrabold leading-6 text-[#26313f] group-hover:text-[#12366f]">{post.title}</h3>
                      <p className="mt-4 line-clamp-2 text-sm leading-6 text-[#4b5563]">{getSummary(post)}</p>
                    </div>
                  </Link>
                ))}
              </div>
        ...
        <section className="pr-container pb-16">
          <div className="pr-section-rule mb-8">
            <h2 className="pr-section-title">All Releases</h2>
          </div>

          {remaining.length ? (
            <div className="grid gap-x-8 gap-y-10 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className={`group block ${index % 3 === 0 ? 'xl:col-span-1' : ''}`}>
                  <div className="aspect-[16/10] overflow-hidden rounded-lg bg-[#eaf2fb]">
                    <img src={getImage(post)} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                  </div>
                  <p className="mt-4 text-xs uppercase text-[#26313f]">{getCategory(post, label)}</p>
                  <h3 className="mt-2 text-xl font-extrabold leading-7 text-[#26313f] group-hover:text-[#12366f]">{post.title}</h3>
                  <p className="mt-3 line-clamp-3 text-sm leading-6 text-[#4b5563]">{getSummary(post)}</p>
                </Link>
              ))}
            </div>
          ) : !lead ? (
            <div className="border border-dashed border-[#9aa0a6] bg-white p-12 text-center">
              <Search className="mx-auto h-8 w-8" />
              <h2 className="mt-4 text-3xl font-extrabold">No releases found</h2>
              <p className="mt-2 text-sm text-[#4b5563]">Try another category or refresh after new content is published.</p>
            </div>
          ) : null}

          <div className="mt-10 flex items-center justify-center gap-0">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="border border-[#9aa0a6] bg-white px-5 py-3 text-xs font-bold uppercase">Previous</Link> : null}
            <span className="border-y border-[#12366f] bg-[#12366f] px-5 py-3 text-xs font-bold uppercase text-white">Page {page} / {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="border border-[#9aa0a6] bg-white px-5 py-3 text-xs font-bold uppercase">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
